...
class Widevine(General):
    ...
    dl_links = {
        ...
        "x86_64": {
            ...
            "12.0.0": [
                "https://github.com/supremegamers/vendor_google_proprietary_widevine-prebuilt/archive/3bba8b6e9dd5ffad5b861310433f7e397e9366e8.zip",
    ...
    def copy(self):
        ...
        shutil.copytree(
            ...
            os.path.join(self.copy_dir, "vendor"),
            dirs_exist_ok=True,
        )
        ...
        if "x86" in self.machine[0] and self.android_version == "11.0.0":
            ...
            os.symlink(
                "./libprotobuf-cpp-lite-3.9.1.so",
                os.path.join(
                    self.copy_dir, "vendor", "lib64", "libprotobuf-cpp-lite.so"
                ),
            )
        ...
        for file in os.listdir(os.path.join(self.copy_dir, "vendor", "etc", "init")):
            if file.endswith(".rc"):
                os.chmod(
                    os.path.join(self.copy_dir, "vendor", "etc", "init", file), 0o644
                )
